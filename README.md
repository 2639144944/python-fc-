...
> 思路：获取网站源码 --> 解析源码，提取所需标签内容 --> 爬取内容保存到excel表格中 --> 将获得数据保存到sql数据库
...
>get请求思路：分析网页(f12) --> 引入模块 --> 模拟打开网页（传递发送数据） --> 接受数据
>
>post请求思路：分析网页(f12) --> 引入模块 --> 封装信息分析网页(f12) --> 引入模块 --> 模拟打开网页（传递发送数据） --> 接受数据模拟打开网页（传递发送数据） --> 接受数据

```python
#解析源码函数，接受一个网址，返回包含源码的列表
def getHtml(url):
    try:
...
>思路：引入模块 --> 解析源码 --> 获得需求信息的标签 --> 设定正则判断规则 --> 获得标签中需求的内容  --> 封装内容

```python
#获得指定标签内的内容,接受源码列表，输出内容列表
def getText(html):
...
>思路：创建表格 --> 创建工作表 --> 写入数据 --> 关闭保存

```python
#将爬取内容保存到excel表格中，接受内容列表，生成excel表格
def getExcel(list):
...
>思路：创建或打开数据库 --> 创建游标  --> 将sql语句交给游标执行 -->  提交数据库操作 --> 退出并关闭数据库

```python
#将获得数据保存到sql数据库,接收数据库名，表单名，数据列表
